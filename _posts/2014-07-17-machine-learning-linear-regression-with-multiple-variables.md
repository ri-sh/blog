...
[![](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img_scatter.png)](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img_scatter.png "Machine Learning: Linear Regression With Multiple Variables")
...
![J\(\\theta\) = \\frac{1}{2m}\\sum_{i=1}^{m}\(h_{\\theta}\(x^{\(i\)}\) - y^{\(i\)}\) ^2](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img1.png)

where the hypothesis ![h_{\\theta}\(x^{\(i\)}\)](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img2.png) is given by the linear model

![h_{\\theta}\(x^{\(i\)}\) = \\theta^{T}x = \\theta_{0} + \\theta_{1}x](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img3.png)

Note that the parameters of our model are the ![\\theta_{j}](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img4.png) values. These are the values that need to be adjusted to minimize the cost ![J\(\\theta\)](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img5.png). One way to do this is to use the batch gradient descent algorithm. In batch gradient descent, each iteration performs the update:

![\\theta_{j} := \\theta_{j} - \\alpha\\frac{1}{m}\\sum_{i=1}^{m}\(h_{\\theta}\(x^{\(i\)}\) - y^{\(i\)}\)x_{j}^{\(i\)}](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img6.png)

The update happens simultaneously for all j. With each iteration of gradient descent the parameters ![\\theta_{j}](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img7.png) come closer to the optimal values that will achieve the lowest cost ![J\(\\theta\)](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img8.png).

Instead of iterating, we could use vectorization to solve the problem quicker. The normal equation is applied in this case:

![\\theta = \(x^{T}x\)^{-1}x^{T}y](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img9.png)
...
Let us compute the cost first. Say we have a matrix  _X_ which contains the training examples. Let there be  _m_ training examples and  _n_ features. Our matrix  _X_ will have one example in each row, thus having  _m_ rows. Also let us add 1 before the first column to each row of  _X_. Thus the dimensions of  _X_ will be ![m X \(n+1\)](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img10.png). Let  _y_ be the output vector of size ![m X 1](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img11.png). Finally, our ![\\theta](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img12.png) will be a ![\(n + 1\) X 1](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img13.png) vector denoting the parameters of our regression model.

Here’s the octave function to compute the cost of using the given ![\\theta](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img14.png) parameters.
...
In the above code, ![X * \\theta](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img15.png) computes the value of ![h_{\\theta}\(x^{\(i\)}\) = \\theta_{0} + \\theta_{1}x](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img16.png). We then subtract the value of  _y_ for that training example. This value is then squared, summed and subsequently reduced by a factor of 2m.

The value of cost function can be used for debugging the iterations when the iterative batch gradient algorithm is used. A continuous reduction of the cost is a sign that the gradient descent is working properly. If the cost rises continuously instead then there is some error in the implementation – perhaps the ![\\alpha](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img17.png) value is too large?
...
In the iterative gradient descent algorithm, one has to guess an ![\\alpha](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img18.png) and also decide upon the number of iterations. It might take multiple runs of the algorithm to settle down on a suitable value for the above variables. Using the normal equation obviates the need to guess these values. It computes the value of ![\\theta](/blog/assets/img/migrated/machine-learning-linear-regression-with-multiple-variables/img19.png) in one shot. The code for the same is:
